...
class Runner(BaseRunner):
    ...
    def _get_reg_dataloader(self, config):
        augments = train_util.parse_augments(config["augments"])
        if config["distributed"]:
            config["dataloader_args"]["batch_size"] //= self.world_size

        data_config = config["data"]["regularization"]
        vocabulary = config["vocabulary"]
        ...
        caption_info = json.load(open(data_config["caption_file"], "r"))["audios"]
        cap_id_to_condition = train_util.load_dict_from_csv(
            data_config["caption_condition"], ("cap_id", "specificity"))

        dataset = ac_dataset.RandomConditionDataset(
            raw_audio_to_h5,
            fc_audio_to_h5,
            attn_audio_to_h5,
            caption_info,
    ...
    def train(self, config, **kwargs):
        ...
        # get regularization dataloader
        reg_dataloader = self._get_reg_dataloader(conf)
        reg_dataiter = iter(reg_dataloader)
        ...
        def _train_batch(engine, batch):
            ...
            with torch.enable_grad():
                ...
                optimizer.zero_grad()
                # step two, alleviate overfitting
                nonlocal reg_dataiter
                try:
                    reg_batch = next(reg_dataiter)
                except StopIteration:
                    reg_dataiter = iter(reg_dataloader)
                    reg_batch = next(reg_dataiter)
                reg_output = self._forward(
                    model, reg_batch, "train",
                    ss_ratio=conf["ss_args"]["ss_ratio"]
                )
                _, word_loss, condition_loss = loss_fn(reg_output)
                loss = conf["reg_alpha"] * word_loss + conf["condition_alpha"] * condition_loss
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), conf["max_grad_norm"])
                optimizer.step()
