...
class Runner(BaseRunner):
    ...
    def train(self, config, **kwargs):
        ...
        dscrm_dump = torch.load(Path(conf["discriminator"]) / "best.pth", map_location="cpu")
        dscrm_conf = train_util.parse_config_or_kwargs(Path(conf["discriminator"]) / "config.yaml")
        dscrm = getattr(captioning.models.hm_classifier, dscrm_conf["model"])(len(vocabulary), **dscrm_conf["model_args"])
        dscrm.load_state_dict(dscrm_dump["model"])
        dscrm = dscrm.to(self.device)
        for param in dscrm.parameters():
    ...
    # def predict(self, 
        ...
        # return super().predict(
            # experiment_path, save_type, raw_feat_csv,
            # fc_feat_csv, attn_feat_csv, caption_output,
            # return_pred, condition=condition, **kwargs)
    ...
    # def evaluate(self,
        ...
        # return super().evaluate(
            # experiment_path, task, save_type,
            # raw_feat_csv, fc_feat_csv, attn_feat_csv,
            # caption_file, caption_output, score_output,
