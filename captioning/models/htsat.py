...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
        ...
        if pretrained is None:
            ...
            self.load_CLAP(pretrained)


    def load_CLAP(self, pretrained):
        ckpt = torch.load(pretrained, "cpu")["state_dict"]
        filtered_dict = {}
        for k in ckpt:
            if k.startswith("module.audio_branch"):
                filtered_dict[k.replace("module.audio_branch.", "")] = ckpt[k]
        model_dict = self.state_dict()
        pretrained_dict = {
            k: v for k, v in filtered_dict.items() if (k in model_dict) and (
    ...
    def forward(self, input_dict):
        ...
        x = self.audio_feats_extractor(waveform)
        ...
        # x = self.head(x)
        attn_emb = output_dict["fine_grained_embedding"]
        emb_lens = [attn_emb.shape[1]] * attn_emb.shape[0]
        emb_lens = torch.as_tensor(emb_lens)
        out = {
            "fc_emb": output_dict["embedding"],
            "attn_emb": attn_emb,
            "attn_emb_len": emb_lens
        }
        return out
