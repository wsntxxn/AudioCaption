...
class Runner(BaseRunner):
    ...
    def _get_dataloaders(config, vocabulary):
        ...
        return trainloader, cvloader, {"cv_key2refs": cv_key2refs}
    ...
    def _forward(self, model, batch, tf=None, mode="train", **kwargs):
        ...
        if mode == "sample":
            ...
            sent_embeds = convert_tensor(sent_embeds.float(),
                                         device=self.device,
                                         non_blocking=True)
            output = model(sent_embeds, mode="sample", **kwargs)
            return output
        ...
        if tf:
            output = model(sent_embeds, caps, cap_lens)
        else:
            output = model(sent_embeds, mode="sample", max_length=max(cap_lens))
    ...
    def train(self, config, **kwargs):
        ...
        def _inference(engine, batch):
            ...
            with torch.no_grad():
                ...
                output["loss"] = criterion(output["probs"], output["targets"]).item()
